	...
	if(isset($_POST['customerDetailsCustomerID'])) {
		...
		if(isset($customerDetailsCustomerFullName) && isset($customerDetailsCustomerMobile) && isset($customerDetailsCustomerAddress)) {
			...
			if(empty($customerDetailsCustomerID)){
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Please enter the UserID to update that user.</div>';
				exit();
			}
			...
			if($customerIDSelectStatement->rowCount() > 0) {
				...
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>UserID does not exist in DB. Therefore, update not possible.</div>';
				exit();
			}
